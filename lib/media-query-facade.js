...
Emitter.prototype.on = function(query, callback, context){
  var queryObject = this._getQuery(query) || this._createQuery(query);
...
Emitter.prototype.off = function(query, callback, context){
  if (!arguments.length) return each(this.queries, this._removeQuery, this);
  if (!callback && !context) return this._removeQuery(this.aliases[query] || query);
...
Emitter.prototype._removeQuery = function(value, query){
  if (isString(value)) query = value;
  var queryObject = this.queries[query];
...
Emitter.prototype._removeHandler = function(query, callback, context){
  var handlers = this._getQuery(query).handlers;
  ...
  if (!handlers.length) this._removeQuery(query);
};

Emitter.prototype._createQuery = function(query){
...
Emitter.prototype._getQuery = function(query){
  return this.queries[this.aliases[query] || query];
};
